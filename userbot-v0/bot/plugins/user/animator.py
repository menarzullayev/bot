...
async def update_name_bio(context: AppContext, client, **kwargs):
    ...
    if not account_id:
        return logger.error("Animator (name/bio): `account_id` topilmadi.")
...
async def update_pfp_clock(context: AppContext, client, **kwargs):
    account_id = kwargs.get('account_id')
    if not account_id:
        return logger.error("Animator (pfp): `account_id` topilmadi.")
    ...
    try:
        ...
        if pfp_text == context.state.get(last_pfp_key):
            return
...
async def update_random_profile(context: AppContext, client, **kwargs):
    account_id = kwargs.get('account_id')
    if not account_id:
        return logger.error(f"Animator (random): `account_id` topilmadi.")
...
async def animator_toggle_cmd(event: Message, context: AppContext):
    if not (event.text and event.client):
        return
    account_id = await get_account_id(context, event.client)
    if not account_id:
        return await event.edit(format_error("Akkaunt ID topilmadi."), parse_mode='html')
...
async def animator_random_list_cmd(event: Message, context: AppContext):
    ...
    if command == "add":
        ...
        if not items:
            return await event.edit(f"ℹ️ Tasodifiy `{item_type}` ro'yxati bo'sh.", parse_mode='html')
        ...
        if not value.isdigit():
            return await event.edit(format_error("O'chirish uchun element ID'sini kiriting."), parse_mode='html')
